            ...
            public void Eject(List<Item> items)
            {
                foreach(Item item in items)
                {
                    if(item.gameObject != null)
                    {
                        ...
                        {
                            /*curObject.AddComponent<EntityState>();
                            curObject.GetComponent<EntityState>().itemValue = _items[0];
                            curObject.GetComponent<EntityState>().entityType = EntityType.Object;
                            curObject.GetComponent<EntityState>().maxHealth = 100;*/
                        }
                        curObject.transform.position = GetComponentInParent<CameraState>().transform.position + GetComponentInParent<CameraState>().transform.forward;
                        curObject.transform.rotation = GetComponentInParent<CameraState>().transform.rotation;
